...
Genome makeDefaultGenome(const std::unordered_map<std::string, double>& gene_thresholds,
    ...
    Gene tp53("TP53", Gene::State::PlusPlus, tp53_th, tp53_k, verbose, logger);
    Gene brca1("BRCA1", Gene::State::PlusMinus, brca1_th, brca1_k, verbose, logger);
    std::unordered_map<std::string, Gene> genes{{tp53.name(), tp53}, {brca1.name(), brca1}};
    return Genome(std::move(genes), verbose, logger);
}
