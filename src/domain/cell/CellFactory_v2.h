...
/// Factory for creating AgenticCell_v2 instances
/// Encapsulates construction logic and default parameters
class CellFactory_v2 {
public:
    /// Create a normal AgenticCell_v2 with standard parameters
    static std::unique_ptr<AgenticCell_v2> createNormalCell(
        const Genome& genome,
        const ports::ILoggerPtr& logger = nullptr);

    /// Create an AgenticCell_v2 with custom parameters
    static std::unique_ptr<AgenticCell_v2> createCustomCell(
        const Genome& genome,
        double neoplasm_k,
